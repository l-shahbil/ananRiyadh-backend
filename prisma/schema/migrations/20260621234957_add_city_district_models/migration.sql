...
-- 3. مدينة وحي احتياطيان لأي سجل ناقص البيانات
INSERT INTO "cities" ("id", "nameAr", "nameEn") VALUES ('fallback_city', 'غير محدد', 'Unspecified');
INSERT INTO "districts" ("id", "nameAr", "cityId") VALUES ('fallback_district', 'غير محدد', 'fallback_city');

-- 4. تعبئة المدن الفعلية من القيم القديمة
...
-- 6. تحديث cityId لكل سجل (احتياطي إن لم يوجد تطابق)
UPDATE "Listing" l
SET "cityId" = COALESCE(
    (SELECT c."id" FROM "cities" c WHERE c."nameAr" = l."city"),
    'fallback_city'
);

-- 7. تحديث districtId لكل سجل (احتياطي إن لم يوجد تطابق)
UPDATE "Listing" l
SET "districtId" = COALESCE(
    (SELECT d."id" FROM "districts" d WHERE d."nameAr" = l."district" AND d."cityId" = l."cityId"),
    'fallback_district'
);

-- 8. حذف الأعمدة القديمة وفرض NOT NULL
